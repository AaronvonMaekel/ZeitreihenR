...
Assume we want to make a prediction for the next trading day (August 12th 2024). This can be done using the Durbin Levinson Algorithm or the Innovations Algorithm. In order to use them however, we will need to turn our raw data into a TimeSeries object. This can be done using the conversion with **as**: 
...
Note that the actual closing price was 39357. Both predictions are close to this, but did not quite get the exact closing price. This underlines the fact that making predictions for a time series (or the stock market) is non-trivial and needs some thoughtful consideration.
